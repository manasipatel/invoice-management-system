...
import { saveInputTokens } from "../db/queries";
...
export const cacheMiddleware: LanguageModelV1Middleware = {
  wrapGenerate: async ({ doGenerate, params }) => {
    ...
    if (cached && cached !== null) {
      console.log("Cache Hit", cached?.usage?.promptTokens);
      /// store the amount of tokens saved in input tokens table
      await saveInputTokens({
        id: generateUUID(),
        chatId: null,
        messageId: null,
        tokens: cached?.usage?.promptTokens || 0,
      });
      return {
        ...cached,
        response: {
  ...
  wrapStream: async ({ doStream, params }) => {
    ...
    if (cached && cached !== null) {
      console.log(
        "Cache Hit",
        cached?.[cached?.length - 1].usage?.promptTokens
      );
      /// store the amount of tokens saved in input tokens table
      await saveInputTokens({
        id: generateUUID(),
        chatId: null,
        messageId: null,
        tokens: cached?.[cached?.length - 1].usage?.promptTokens || 0,
      });
